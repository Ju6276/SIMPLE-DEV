...
TASK="${TASK:-simple/G1WholebodyOpenOvenTeleop-v0}"
...
Examples:
  $(basename "$0")
  $(basename "$0") simple/G1WholebodyOpenOvenTeleop-v0
  $(basename "$0") simple/G1WholebodyOpenOvenTeleop-v0 10090
  $(basename "$0") simple/G1WholebodyOpenOvenTeleop-v0 10090 10
...
if [[ $# -gt 3 ]]; then
  echo "[run_eval_vlajepa_all_levels] too many positional arguments" >&2
  usage >&2
  exit 1
fi

SCRIPT_PATH="$ROOT_DIR/scripts/run_eval_vlajepa_clean.sh"
if [[ ! -f "$SCRIPT_PATH" ]]; then
  echo "[run_eval_vlajepa_all_levels] missing script: $SCRIPT_PATH" >&2
  exit 1
fi

echo "[run_eval_vlajepa_all_levels] repo: $ROOT_DIR"
echo "[run_eval_vlajepa_all_levels] task: $TASK"
echo "[run_eval_vlajepa_all_levels] policy: $POLICY"
echo "[run_eval_vlajepa_all_levels] server: ${HOST}:${PORT}"
echo "[run_eval_vlajepa_all_levels] num episodes per level: $NUM_EPISODES"
echo "[run_eval_vlajepa_all_levels] levels: $LEVELS"
echo "[run_eval_vlajepa_all_levels] eval dir root: $EVAL_DIR_ROOT"
...
if [[ "$CLEAR_OLD_RESULTS" == "1" ]]; then
  for level in $LEVELS; do
    ...
    if [[ -d "$level_task_dir" ]]; then
      echo "[run_eval_vlajepa_all_levels] removing old results: $level_task_dir"
...
echo
echo "[run_eval_vlajepa_all_levels] finished all levels for $TASK"
for level in $LEVELS; do
  echo "[run_eval_vlajepa_all_levels] ${level} log: $ROOT_DIR/${EVAL_DIR_ROOT}/${level}/eval_latest.log"
  echo "[run_eval_vlajepa_all_levels] ${level} stats: $ROOT_DIR/${EVAL_DIR_ROOT}/${level}/eval_stats.txt"
done
