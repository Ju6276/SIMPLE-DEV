...
Examples:
  $(basename "$0")
  $(basename "$0") level-0
  $(basename "$0") level-1 22085
  $(basename "$0") level-2 22085 -- --num-episodes 4
...
if [[ ${#POSITIONAL[@]} -gt 2 ]]; then
  echo "[run_eval_psi0_clean] too many positional arguments" >&2
  usage >&2
  exit 1
fi
...
if [[ ! -d "$DATA_DIR" ]]; then
  echo "[run_eval_psi0_clean] data dir not found: $DATA_DIR" >&2
  exit 1
fi

if ! command -v uv >/dev/null 2>&1; then
  echo "[run_eval_psi0_clean] missing required command: uv" >&2
  exit 1
fi
...
if [[ -r /proc/sys/fs/inotify/max_user_watches ]]; then
  ...
  if [[ "${current_watches}" -lt 262144 ]]; then
    echo "[run_eval_psi0_clean] warning: fs.inotify.max_user_watches=${current_watches} is low for Isaac Sim." >&2
    echo "[run_eval_psi0_clean] recommendation: sudo sysctl fs.inotify.max_user_watches=524288" >&2
  fi
fi
...
echo "[run_eval_psi0_clean] repo: $ROOT_DIR"
echo "[run_eval_psi0_clean] task: $TASK"
echo "[run_eval_psi0_clean] policy: $POLICY"
echo "[run_eval_psi0_clean] split: $SPLIT"
echo "[run_eval_psi0_clean] data dir: $DATA_DIR"
echo "[run_eval_psi0_clean] eval dir: $EVAL_DIR"
echo "[run_eval_psi0_clean] server: ${HOST}:${PORT}"
echo "[run_eval_psi0_clean] mujoco gl: $MUJOCO_GL_VALUE"
echo "[run_eval_psi0_clean] clearing CONDA/PYTHON/LD runtime pollution before launch"
