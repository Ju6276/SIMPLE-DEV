...
def _websocket_ping_env(name: str, default: Optional[float]) -> Optional[float]:
    raw = os.getenv(name)
...
class WebsocketPolicyServer:
    ...
    async def run(self):
        async with websockets.asyncio.server.serve(
            ...
            compression=None,
            max_size=None,
            ping_interval=_websocket_ping_env("STARVLA_WS_PING_INTERVAL", 20.0),
            ping_timeout=_websocket_ping_env("STARVLA_WS_PING_TIMEOUT", 20.0),
        ) as server:
