    ...
    def forward(
        ...
        *,
        ...
        if robot_state.ndim != 2 or int(robot_state.shape[1]) != int(self.state_dim):
            raise ValueError(f"expected robot_state to be (B,{int(self.state_dim)}), got shape={tuple(robot_state.shape)}")
...
class Qwen3VLDraftChunkHead(DraftChunkHead):
    """FLASH draft head using one Qwen3-VL text decoder layer over prefix embeddings."""
    ...
    def __init__(
        ...
        *,
        ...
        qwen_config._attn_implementation = self.attn_implementation  # noqa: SLF001

        self._state_token = nn.Linear(int(self.state_dim), int(self.hidden_size))
        self._action_queries = nn.Embedding(int(self.chunk_m), int(self.hidden_size))
        self._qwen_block = Qwen3VLTextDecoderLayer(qwen_config, layer_idx=0)
        self._rotary_emb = Qwen3VLTextRotaryEmbedding(qwen_config)
        self._action_head = nn.Linear(int(self.hidden_size), int(self.out_dim))

    def init_from_vlm_layer(self, layer: nn.Module) -> None:
        self._qwen_block.load_state_dict(layer.state_dict(), strict=True)
    ...
    def forward(
        ...
        *,
        ...
        b = int(robot_state.shape[0])
        block_dtype = self._qwen_block.self_attn.q_proj.weight.dtype
        prefix_embs = prefix_embs.to(dtype=block_dtype)
        state_token = self._state_token(robot_state.to(dtype=self._state_token.weight.dtype))[:, None, :].to(dtype=block_dtype)
        query_ids = torch.arange(int(self.chunk_m), device=prefix_embs.device, dtype=torch.long)[None, :].expand(b, -1)
        query_tokens = self._action_queries(query_ids).to(dtype=block_dtype)
        ...
        position_ids = self._build_position_ids(prefix_pad_masks=prefix_pad_masks)
        position_embeddings = self._rotary_emb(hidden_states, position_ids)
        hidden_states = self._qwen_block(
            hidden_states,
            attention_mask=attention_mask,
            position_ids=position_ids,
