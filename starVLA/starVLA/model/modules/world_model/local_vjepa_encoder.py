...
# Backward-compatible alias for the HWM framework source tree.
VJEPA2LocalEncoder = VJEPA2LocalBackbone
...
def load_local_vjepa_encoder_checkpoint(model: VJEPA2LocalEncoderModel, checkpoint_path: str | Path) -> None:
    ...
    source_state = checkpoint.get("ema_encoder") or checkpoint.get("encoder")
    if not isinstance(source_state, dict):
        raise KeyError(
            f"V-JEPA checkpoint `{checkpoint_path}` does not contain an `ema_encoder` or `encoder` state_dict."
        )
