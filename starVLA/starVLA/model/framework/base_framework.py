...
def _import_framework_module_by_id(framework_id: str) -> None:
    framework_dir = Path(__file__).resolve().parent
    candidates = list(framework_dir.rglob(f"{framework_id}.py"))
    for candidate in candidates:
        rel = candidate.relative_to(framework_dir).with_suffix("")
        module = ".".join(("starVLA", "model", "framework", *rel.parts))
...
def build_framework(cfg): # The single entry point for building different model frameworks
    ...
    if framework_id not in FRAMEWORK_REGISTRY._registry:
        _import_framework_module_by_id(framework_id)
