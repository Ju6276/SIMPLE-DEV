...
def resolve_model_id_or_path(model_id_or_path: Any) -> Any:
    ...
    jepamemory_root = Path("/home/d013/桌面/JEPAMEMORY")
    fallback_candidates = []

    if not candidate.is_absolute():
        fallback_candidates.append((repo_root / candidate).resolve())
        fallback_candidates.append((jepamemory_root / candidate).resolve())
    ...
    fallback_candidates.append(jepamemory_root / candidate.name)
    fallback_candidates.append(jepamemory_root / "VJEPA21" / candidate.name)
    fallback_candidates.append(jepamemory_root / "playground" / "Pretrained_models" / candidate.name)
...
def read_mode_config(pretrained_checkpoint):
    ...
    if os.path.isfile(pretrained_checkpoint):
        ...
        framework_cfg = global_cfg.get("framework", {}) if isinstance(global_cfg, dict) else {}
        if isinstance(framework_cfg, dict):
            qwenvl_cfg = framework_cfg.get("qwenvl", {})
            if isinstance(qwenvl_cfg, dict) and "base_vlm" in qwenvl_cfg:
                qwenvl_cfg["base_vlm"] = resolve_model_id_or_path(qwenvl_cfg["base_vlm"])
            vj2_cfg = framework_cfg.get("vj2_model", {})
            if isinstance(vj2_cfg, dict) and "encoder_checkpoint" in vj2_cfg:
                vj2_cfg["encoder_checkpoint"] = resolve_model_id_or_path(vj2_cfg["encoder_checkpoint"])
            if isinstance(vj2_cfg, dict) and "base_encoder" in vj2_cfg:
                vj2_cfg["base_encoder"] = resolve_model_id_or_path(vj2_cfg["base_encoder"])
