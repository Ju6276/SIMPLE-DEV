...
def override_wbc_config(
    ...
    key_to_value = {
        ...
        "EVAL_SERVER_ENABLED": getattr(config, "eval_server_enabled", False),
        "EVAL_SERVER_HOST": getattr(config, "eval_server_host", "127.0.0.1"),
        "EVAL_SERVER_PORT": getattr(config, "eval_server_port", 5561),
    }
    ...
    if config.robot_scene_override:
        wbc_config["ROBOT_SCENE"] = config.robot_scene_override

    camera_randomization_config = {
        "enabled": config.camera_randomization_enabled,
        "camera_name": config.camera_randomization_camera_name,
        "pos_jitter_x": config.camera_pos_jitter_x_meters,
        "pos_jitter_y": config.camera_pos_jitter_y_meters,
        "pos_jitter_z": config.camera_pos_jitter_z_meters,
        "roll_jitter": config.camera_roll_jitter_radians,
        "pitch_jitter": config.camera_pitch_jitter_radians,
        "yaw_jitter": config.camera_yaw_jitter_radians,
    }
    if any(
        camera_randomization_config[key] > 0.0
        for key in (
            "pos_jitter_x",
            "pos_jitter_y",
            "pos_jitter_z",
        ...
        wbc_config["CAMERA_RANDOMIZATION"] = camera_randomization_config

    robot_start_randomization_config = {
        "enabled": config.robot_start_randomization_enabled,
        "min_meters": config.robot_start_min_meters,
        "max_meters": config.robot_start_max_meters,
    }
    if robot_start_randomization_config["enabled"]:
        wbc_config["ROBOT_START_RANDOMIZATION"] = robot_start_randomization_config
...
class BaseConfig(ArgsConfigTemplate):
    ...
    robot_scene_override: str = ""
    """Optional absolute or repo-relative MuJoCo scene XML override."""
...
class SimLoopConfig(BaseConfig):
    ...
    camera_randomization_enabled: bool = False
    """Whether to randomize the MuJoCo camera at runtime on reset."""

    camera_randomization_camera_name: str = "head_camera"
    """MuJoCo camera name for runtime camera randomization."""

    camera_pos_jitter_x_meters: float = 0.0
    """Uniform +/- runtime position jitter in meters for camera x."""

    camera_pos_jitter_y_meters: float = 0.0
    """Uniform +/- runtime position jitter in meters for camera y."""

    camera_pos_jitter_z_meters: float = 0.0
    """Uniform +/- runtime position jitter in meters for camera z."""

    camera_roll_jitter_radians: float = 0.0
    """Uniform +/- runtime roll jitter in radians."""

    camera_pitch_jitter_radians: float = 0.0
    """Uniform +/- runtime pitch jitter in radians."""

    camera_yaw_jitter_radians: float = 0.0
    """Uniform +/- runtime yaw jitter in radians."""

    robot_start_randomization_enabled: bool = False
    """Whether to randomize the robot start position at reset time."""

    robot_start_min_meters: float = 0.02
    """Minimum forward/back offset applied at reset time."""

    robot_start_max_meters: float = 0.07
    """Maximum forward/back offset applied at reset time."""

    eval_server_enabled: bool = False
    """Enable a small ZMQ REP server for automated eval reset/status commands."""

    eval_server_host: str = "127.0.0.1"
    """Bind host for the automated eval server."""

    eval_server_port: int = 5561
    """Bind port for the automated eval server."""
