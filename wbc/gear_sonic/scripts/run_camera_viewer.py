...
class CameraViewerConfig:
    ...
    record_start_file: Optional[str] = None
    """Optional file trigger; creating this file starts recording."""

    record_stop_file: Optional[str] = None
    """Optional file trigger; creating this file stops recording."""
...
def main(config: CameraViewerConfig):
    ...
    output_path = config.output_path or os.getenv("SONICSTAR_CAMERA_OUTPUT_PATH", "")
    output_dir = Path(output_path) if output_path else Path("camera_recordings")
    record_start_file_raw = config.record_start_file or os.getenv("SONICSTAR_CAMERA_RECORD_START_FILE", "")
    record_stop_file_raw = config.record_stop_file or os.getenv("SONICSTAR_CAMERA_RECORD_STOP_FILE", "")
    record_start_file = Path(record_start_file_raw) if record_start_file_raw else None
    record_stop_file = Path(record_stop_file_raw) if record_stop_file_raw else None
    ...
    if record_start_file is not None:
        print(f"Recording start trigger file: {record_start_file}")
    if record_stop_file is not None:
        print(f"Recording stop trigger file: {record_stop_file}")
    print("Controls: R = start/stop recording, Q = quit")

    def start_recording(current_images: dict[str, np.ndarray], reason: str) -> None:
        nonlocal is_recording, video_writers, frame_count, recording_start_time, recording_dir
        if is_recording:
            return
        ...
        for name in camera_names:
            img = current_images.get(name)
            if img is not None:
                h, w = img.shape[:2]
                path = recording_dir / f"{name}.mp4"
                writer = cv2.VideoWriter(str(path), fourcc, config.fps, (w, h))
                if not writer.isOpened():
                    print(f"WARNING: failed to open video writer for {path}")
                    continue
                video_writers[name] = writer

        if not video_writers:
            print("WARNING: no camera video writers opened; recording not started")
            return
        is_recording = True
        recording_start_time = time.time()
        frame_count = 0
        print(f"Recording started ({reason}): {recording_dir}", flush=True)

    def stop_recording(reason: str) -> None:
        nonlocal is_recording, video_writers
        if not is_recording:
            return
        is_recording = False
        ...
        print(
            f"Recording stopped ({reason}) - {duration:.1f}s, {frame_count} frames "
            f"-> {recording_dir}",
            flush=True,
        )
    ...
    try:
        while True:
            ...
            if record_start_file is not None and record_start_file.exists() and not is_recording:
                start_recording(image_data["images"], "trigger_file")
                try:
                    record_start_file.unlink()
                except OSError:
                    pass
            if record_stop_file is not None and record_stop_file.exists() and is_recording:
                stop_recording("trigger_file")
                try:
                    record_stop_file.unlink()
                except OSError:
                    pass
            ...
            if key == ord("q"):
                ...
                if not is_recording:
                    start_recording(image_data["images"], "keyboard")
                else:
                    stop_recording("keyboard")
